...
class ControllerApiLogin extends Controller {
	...
	public function addApiUser() {
        ...
        if(isset($this->request->post['username']) && isset($this->request->post['key'])) {
            ...
            $api_info = $this->model_account_api->addApiUser($username, $key);
            if($api_info) {
                $json['id'] = $api_info;
                $json['success'] = $this->language->get('add_api_user_success');
            }
        } else {
            $json['add_api_user_error'] = $this->language->get('add_api_user_error');
        }
        $this->response->addHeader('Content-Type: application/json');
        $this->response->setOutput(json_encode($json));
    }
